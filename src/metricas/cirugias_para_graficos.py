...
#os.remove(f"datos_cirugias/{eliminar}.gzip")
...
for i,j in zip([menor, menor_año], [menor, "acumulado"]): 
    ...
    where (CONVERT(DATETIME,a13.exp_mes, "mm-YYYY") >= date("{i}") and CONVERT(DATETIME,a13.exp_mes, "mm-YYYY") <= date("{mayor}")
    ...
    final.to_parquet(f'datos_cirugias/{j}.gzip',compression='gzip') 
